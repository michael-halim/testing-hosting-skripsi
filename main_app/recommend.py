...
def train_weighted_matrix(user_id , total_highest_cbf = TOTAL_HIGHEST_CBF, total_lowest_cbf = TOTAL_LOWEST_CBF):
    ...
    for _id in cbf_product_ids:
        if max_id == _id:
            ...
            if distance_object.count() > 0:
                distance_object = distance_object[0]
            else:
                continue
...
def train_model(is_all_user=True, user_id=None):
    is_show_score = True
    ...
    if (not is_all_user) and (not user_id is None):
        unique_user = User.objects.filter(id=user_id)
